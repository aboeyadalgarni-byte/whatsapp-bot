...
import { fetchUrlText } from "./url.js";
...
const MENU =
  "هلا 👋 أنا «سوّها».\n\n" +
  "أرسل رابطًا أو ملفًا، واكتب وش تبي أسوي.\n\n" +
  "أمثلة:\n" +
  "• لخص الرابط\n" +
  "• ترجم للعربي\n" +
...
export async function processUserMessage(message) {
  ...
  if (message.type === "text") {
    ...
    if (/^(ابدأ|start|القائمة|مساعدة|help)$/i.test(text)) return MENU;
    ...
    const action = detectAction(text);
    if (session.url && action) return executeUrlAction(session.url, action);

    return openai
      ? askAssistant(text)
      : "أرسل رابطًا أو ملفًا، واكتب المطلوب مثل: «لخص» أو «ترجم».";
  }

  if (["document", "audio", "video"].includes(message.type)) {
    return extractMediaText(message);
  }

  if (message.type === "image") {
    return "وصلت الصورة 🖼️\nأرسل طلبك مثل: «استخرج النص» أو «اشرح الصورة».";
  }
...
async function executeUrlAction(url, action) {
  ...
  const instruction = {
    summarize: "لخّص المحتوى بالعربية في نقاط قصيرة، واذكر أهم المعلومات فقط.",
    translate: "ترجم المحتوى إلى العربية ترجمة واضحة، مع الحفاظ على الأرقام والأسماء والروابط.",
    extract: "استخرج المعلومات المهمة في قائمة منظمة، ولا تخترع أي معلومة غير موجودة.",
...
async function askAssistant(text) {
  return askModel("أنت مساعد «سوّها» على واتساب. أجب بالعربية السعودية بشكل مختصر وعملي. لا تخترع معلومات.\nطلب المستخدم: " + text);
}
